    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="max-w-xl mx-auto relative group">
        <form onSubmit={handleSubmit} className="relative z-20">
          ...
          <Input
            ...
            className="pl-12 pr-36 h-14 text-lg bg-slate-900/50 border-slate-700 rounded-xl focus:border-blue-500 focus:ring-blue-500/20 text-white placeholder:text-slate-600 shadow-xl"
          />
          <Button
            type="submit"
      ...
      {!submitted && (
        <div className="max-w-2xl mx-auto text-center">
          <p className="text-sm text-slate-500 mb-4 font-medium">인기 전공 (사용자 설정 기준)</p>
            ...
            {popularMajorsLoading ? (
              ...
              popularMajors.map((m) => (
                <Button
                  ...
                  className="rounded-full bg-slate-800/40 border-slate-700 text-slate-300 hover:bg-slate-800 hover:text-blue-400 h-8 text-sm px-4"
                >
                  {m}
                </Button>
              ))
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {sampleMajors.map((m) => (
                    <Button
                      ...
                      size="sm"
                      onClick={() => handleMajorClick(m)}
                      className="rounded-full bg-slate-800/40 border-slate-700 text-slate-400 hover:bg-slate-800 hover:text-blue-400 h-8 text-sm px-4"
                    >
                      {m}
                    </Button>
                  ))}
            ...
            <>
              <div className="flex items-center justify-between px-2">
                <h2 className="text-xl font-bold text-white">
                  <span className="text-blue-400">&quot;{recommendations?.major}&quot;</span> 추천 자격증
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                              ...
                              <div className="flex items-center gap-6 bg-slate-950/40 px-4 py-2 rounded-lg border border-slate-800/50">
                                ...
                                <div className="text-right pl-4 border-l border-slate-800">
                                  <p className="text-[10px] text-slate-500 font-semibold mb-0.5">추천점수</p>
                                  <p className="text-lg font-bold text-blue-400">{rec.score.toFixed(1)}</p>
                                </div>
                              </div>
                            </div>

                            {rec.reason && (
                              <div className="bg-blue-500/5 border border-blue-500/10 rounded-lg p-3 group-hover:bg-blue-500/10 transition-colors">
        ...
        <div className="grid md:grid-cols-3 gap-6 pt-12 max-w-5xl mx-auto">
          ...
          <Card className="bg-slate-900/40 border-slate-800/80 hover:bg-slate-900/60 transition-colors shadow-sm">
            <CardContent className="p-6 space-y-4">
              ...
              <p className="text-sm text-slate-400 leading-relaxed font-medium">
                전공과 관심사를 반영한 시멘틱 검색과 키워드 매칭을 결합해, 자격증 시험 과목과의 연관성을 분석하여 최적의 취득 경로를 제시합니다.
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-slate-900/40 border-slate-800/80 hover:bg-slate-900/60 transition-colors shadow-sm">
            <CardContent className="p-6 space-y-4">
              ...
              <p className="text-sm text-slate-400 leading-relaxed font-medium">
                학년과 취득 자격증을 반영한 난이도 조정과 함께, 전공 맞춤 추천으로 단계별 자격증 취득 로드맵을 제안합니다.
              </p>
            </CardContent>
          </Card>
        </div>
